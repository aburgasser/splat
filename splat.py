...
# plot spectrum compared to best spectrum
    if (kwargs.get('plot',False) != False):
        ...
        spstd.scale(scale)
        splot.plotSpectrum(sp,spstd,colors=['k','r'],\
...
# plot spectrum compared to best spectrum
    if (kwargs.get('plot',False) != False):
        ...
        s.scale(scale)
        splot.plotSpectrum(sp,s,colors=['k','r'],title=sp.name+' vs '+s.name,**kwargs)
...
# read in parameters of available models
    folder = checkLocal(SPLAT_PATH+SPECTRAL_MODEL_FOLDER+kwargs['set']+'/')
    if folder=='':
        raise NameError('\n\nCould not locate spectral model folder {} locally\n'.format(SPECTRAL_MODEL_FOLDER+kwargs['set']+'/'))
    pfile = checkLocal(folder+pfile)
    if pfile=='':
...
# by default assume models come from local splat directory
    local = kwargs.get('local',True)
    online = kwargs.get('online',not local and checkOnline())
    kwargs['folder'] = kwargs.get('folder',SPLAT_PATH+SPECTRAL_MODEL_FOLDER)
    local = not online
    kwargs['local'] = local
    kwargs['online'] = online
    kwargs['model'] = True
...
# generate model filename
    kwargs['filename'] = kwargs['folder']+'_{:.0f}_{:.1f}_{:.1f}_{}_{}_{}_{:.1f}.txt'.\
...
# check plotSpectrum
#    mdl.scale(scale)
    splot.plotSpectrum(sp,colors=['k','r'],title='If this appears everything is OK: close window')
